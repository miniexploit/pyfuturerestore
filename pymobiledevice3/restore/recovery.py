...
class Recovery(BaseRestore):
    def __init__(self, ipsw: BytesIO, device: Device, tss: typing.Mapping = None, sepfwdata=None, sepbuildmanifest=None, bbfwdata=None, basebandbuildmanifest=None, behavior: Behavior = Behavior.Update):
        super().__init__(ipsw, device, tss, sepfwdata, sepbuildmanifest, bbfwdata, basebandbuildmanifest, behavior, logger=logging.getLogger(__name__))
    ...
    def send_component(self, name: str):
        ...
        if self.sepfwdata:
            data = self.sep_build_identity.get_component(name, tss=tss, data=data).personalized_data
        else:    
