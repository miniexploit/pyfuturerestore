...
class IRecv:
    def __init__(self, ecid=None, timeout=0xffffffff, is_recovery=None, get_mode=False):
        self.mode = None  # type: Optional[Mode]
        self._device_info = {}
        self._device = None  # type: Optional[Device]
        self._reinit(ecid=ecid, timeout=timeout, is_recovery=is_recovery, get_mode=get_mode)
    ...
    def _reinit(self, ecid=None, timeout=0xffffffff, is_recovery=None, get_mode=False):
        self._device = None
        self._device_info = {}
        self.mode = None
        self._find(ecid=ecid, timeout=timeout, is_recovery=is_recovery, get_mode=get_mode)
        if get_mode:
            return
    ...
    def _find(self, ecid=None, timeout=0xffffffff, is_recovery=None, get_mode=False):
        start = time.time()
        end = start + timeout
